    ...
    try {
      ...
      if (block.number != blockNumber) {
        ...
        for (let r of receipts) {
          // Storing just (hash, status) to savesendRawT memory.
...
const checkETHBalances = async (config: TPSConfig, deployer: Wallet, nonce: number | null) => {
  ...
  console.log(`[checkETHBalances] ${sender.address} ETH balance: ${ethers.utils.formatEther(balance)}`);
  if (balance.lte(ethers.utils.parseEther("50"))) await batchSendEthers(config, deployer, nonce);
}
